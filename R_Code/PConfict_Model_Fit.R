...
did_df$farc = ifelse(did_df$gr == 3,1,0)
did_df$farc_eln = ifelse(did_df$gr == 4,1,0)
y2012 = data.frame(lower = rep(0,2),
                   upper = rep(0,2),
                   mean = rep(0,2),
                   year = rep(2012,2),
                   Group = c("FARC","FARC-ELN"))

## Violence

reg <- plm(incidents~factor(time) + farc + farc_eln + factor(time):farc + factor(time):farc_eln + factor(dpto):factor(time), data = did_df, effect = "individual",
           index = c("id", "time"), 
           model = "within")
vt2 = coefci(reg,level = .95, vcov=vcovHC(reg,type="HC0",cluster="group"))

vt2 = as.data.frame(vt2[11:30,])
colnames(vt2) = c("lower","upper")
vt2$mean = (vt2$lower+vt2$upper)/2
vt2$year = rep(c(2008:2011,2013:2018),2)
vt2$Group = c(rep("FARC",10),rep("FARC-ELN",10))

vt2 = rbind(vt2,y2012)

bar_vln = ggplot(vt2, aes(x=year, y=mean, group=Group, color=Group, linetype=Group)) +
  geom_line(aes(x=year, y=mean), position = pd) +
  ...
  scale_color_manual(name = "Armed \ngroup",
                     values=c("gray40","gray10"),
                     labels = c("FARC","FARC-ELN")) +
  scale_linetype_manual(name = "Armed \ngroup",
                     values=c("solid","dashed"),
                     labels = c("FARC","FARC-ELN")) +
  xlab("Year") +
  ylab("Slope coefficient (\u03B8)")+
...
reg <- plm(coca~time + farc + farc_eln + factor(time):farc + factor(time):farc_eln + factor(dpto):time, data = did_df, effect = "individual",
           index = c("id", "time"), 
           model = "within")
vt3 = coefci(reg,level = .95, vcov=vcovHC(reg,type="HC0",cluster="group"))

vt3 = as.data.frame(vt3[11:30,])
colnames(vt3) = c("lower","upper")
vt3$mean = (vt3$lower+vt3$upper)/2
vt3$year = rep(c(2008:2011,2013:2018),2)
vt3$Group = c(rep("FARC",10),rep("FARC-ELN",10))

vt3 = rbind(vt3,y2012)

bar_coca = ggplot(vt3, aes(x=year, y=mean, group=Group, color=Group, linetype = Group)) +
